...
class StateManager {
  ...
  initSession({ sessionId, sourcePath, files }) {
    ...
    const existingFiles = sameSource && existing.files ? existing.files : {};
  ...
  renameTrackedFile(oldRelativePath, newRelativePath) {
    if (!this.state?.files?.[oldRelativePath]) return false;
    if (this.state.files[newRelativePath]) return false;
  ...
  markTrackedFileDeleted(relativePath, error = 'File deleted from repository') {
    if (!this.state?.files?.[relativePath]) return false;
