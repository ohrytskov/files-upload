...
function replaceStateFile(tempPath, stateFilePath) {
  try {
    fs.renameSync(tempPath, stateFilePath);
    return;
  } catch (error) {
    // POSIX rename replaces an existing destination atomically. Windows does
    // not, so use a narrow compatibility fallback for the client-side JSON
    // state file while retaining the atomic fast path everywhere else.
    if (
...
class StateManager {
  ...
  saveState() {
    ...
    try {
      ...
      replaceStateFile(tempPath, this.stateFilePath);
    } catch (err) {
      try { fs.unlinkSync(`${this.stateFilePath}.tmp`); } catch (cleanupError) {}
