...
class WebSocketUploaderClient {
  ...
  connectWebSocket() {
    ...
    this.ws.on('open', () => {
      ...
      const initializeSession = () => this.ws.send(JSON.stringify({
        type: 'INIT_SESSION',
        payload: {
      ...
      if (this.authToken) {
        this.ws.send(JSON.stringify({ type: 'AUTH', payload: { token: this.authToken } }));
      } else {
        initializeSession();
      }
    });
  ...
  async handleServerMessage(msg) {
    ...
    switch (type) {
      ...
      case 'FILE_VERIFIED': {
        const { relativePath, serverHash, serverMd5, match } = payload;
