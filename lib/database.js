...
class CloudVaultDatabase {
  ...
  renameRepositoryFile(oldPath, newPath) {
    const transaction = this.db.transaction(() => {
      ...
      if (!record) return;
      // A file may have been removed outside the API, leaving stale metadata
      // at the requested destination. The filesystem rename is already
      // protected against replacing a real destination, so replacing stale
      // metadata here keeps the repository index consistent.
      if (oldPath !== newPath) this.deleteRepositoryFileStatement.run(newPath);
