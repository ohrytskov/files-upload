...
const SESSION_TAKEN_OVER_REASON = 'Session taken over by reconnecting client';
...
async function inspectExistingUpload(fileItem, targetPath) {
  ...
  if (finalStat) {
    if (finalStat.size === fileItem.size) {
      ...
      // prefix. Keep it published until the replacement is fully verified.
      finalStat = null;
    } else if (finalStat.size < fileItem.size) {
      // A final file is already public content, not a resumable staging file.
      // Do not move or delete it just because a new manifest has the same
      // relative path; the replacement will be published after verification.
      finalStat = null;
    } else {
      // Preserve an existing larger file until the replacement is verified.
      finalStat = null;
    }
  }
  ...
  if (partStat && partStat.size === fileItem.size) {
    ...
    if (partHash === fileItem.hash) {
      syncFile(partPath);
      publishVerifiedUpload(partPath, targetPath);
...
function publishVerifiedUpload(partPath, targetPath) {
  // On POSIX this replaces an existing regular file atomically, so readers
  // never observe a partially written replacement. Windows may reject rename
  // over an existing file; retain the same behavior there with a narrow
  // fallback after both paths have been validated.
  getRegularFileStat(targetPath);
  try {
    fs.renameSync(partPath, targetPath);
  } catch (error) {
    if (!['EEXIST', 'EPERM', 'ENOTEMPTY'].includes(error.code)) throw error;
    const existing = getRegularFileStat(targetPath);
    if (existing) removeRegularFile(targetPath);
    fs.renameSync(partPath, targetPath);
  }
}
...
function initWebSocketServer(httpServer, uploadsDir, options = {}) {
  ...
  const maxQueuedMessages = options.maxQueuedMessages ?? config.wsMaxQueuedMessages;
  ...
  const databasePath = path.resolve(options.databasePath ?? path.join(uploadsDir, 'cloudvault.sqlite'));
  const reservedStoragePaths = new Set([
    databasePath,
    `${databasePath}-wal`,
    `${databasePath}-shm`,
    `${databasePath}-journal`
  ]);
  const isReservedUploadPath = relativePath => {
    const normalized = path.posix.normalize(relativePath);
    const resolved = path.resolve(resolvedUploadsDir, ...normalized.split('/'));
    return isInternalUploadFile(path.posix.basename(normalized)) || reservedStoragePaths.has(resolved);
  };
  ...
  const invalidatedSockets = new WeakSet();
  ...
  wss.on('connection', (ws) => {
    ...
    const getSessionFile = (relativePath) => {
      ...
      if (!activeSession || !activeSession.files || !hasOwn(activeSession.files, normalized)) {
    ...
    const processMessage = async (rawMessage, isBinary) => {
      ...
      if (invalidatedSockets.has(ws)) {
        if (ws.readyState === WebSocket.OPEN) ws.close(SESSION_TAKEN_OVER_CLOSE_CODE, SESSION_TAKEN_OVER_REASON);
        return;
      }
        ...
        case 'INIT_SESSION': {
          ...
          try {
            ...
            normalizedFiles = files.map(fileItem => {
              ...
              if (impliedDirectories.has(relativePath)) {
                throw new Error(`File path conflicts with a directory path: ${relativePath}`);
              }
              ...
              for (const segment of segments.slice(0, -1)) {
                ...
                if (seenPaths.has(parentPath)) {
                  throw new Error(`File path conflicts with a directory path: ${parentPath}`);
                }
                impliedDirectories.add(parentPath);
              }
              ...
              if (isReservedUploadPath(relativePath)) {
          ...
          if (activeSessionOwner && activeSessionOwner !== ws) {
            invalidatedSockets.add(activeSessionOwner);
            activeSessionOwner.close(SESSION_TAKEN_OVER_CLOSE_CODE, SESSION_TAKEN_OVER_REASON);
          }
          activeSessionOwner = ws;
          activeSessionId = sessionId;
          ...
          const fileStatuses = Object.create(null);
          for (const key in activeSession.files) {
            ...
            if (invalidatedSockets.has(ws)) return;
    ...
    const drainMessageQueue = () => {
      if (processingMessage || queueClosed) return;
      const nextMessage = messageQueue.shift();
      if (!nextMessage) return;

      processingMessage = true;
      Promise.resolve(processMessage(nextMessage.rawMessage, nextMessage.isBinary))
        .catch(err => {
    ...
    ws.on('message', (rawMessage, isBinary) => {
      ...
      if (messageQueue.length >= maxQueuedMessages) {
        queueClosed = true;
        messageQueue.length = 0;
        try { ws.close(1008, 'Too many queued messages'); } catch (error) {}
        return;
      }

      messageQueue.push({ rawMessage, isBinary });
      drainMessageQueue();
    });
