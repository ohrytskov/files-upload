...
1. **Configurable Hash Generator (`lib/hash-generator.js`)**:
   ...
   - Supports **Windows 11 paths** (e.g., `D:\source-folder`, `C:\Users\...`) and **Linux paths**.
...
- Node.js 22.5 or newer is recommended. The application prefers `better-sqlite3` and can fall back to Node's built-in `node:sqlite` on supported Node versions.
- A writable upload directory with enough space for staged and finalized files.

The checked-in files under `uploads/` are small demo fixtures. New uploads, SQLite files, resumable state, temporary parts, and logs are runtime data and are ignored by Git. Do not place private data in the tracked demo fixtures.
...
node bin/upload-cli.js -p "D:\source-folder" -s "ws://127.0.0.1:3001/ws/upload"
...
- **Commander View** (the default view): Set the left and right panels to any server-local directory, including Windows drives, UNC paths, and mounted flash drives, preview PNGs, download a selected file to your device, and copy files or directories between panels. Select PDFs and choose **Copy as...** to create 300 DPI PNG page images in the opposite panel. See [local Commander operations](docs/local-copy.md).
...
The combined check is also available as `npm run test:all`.
...
├── uploads/                # Demo fixtures; runtime uploads are ignored
├── package.json
